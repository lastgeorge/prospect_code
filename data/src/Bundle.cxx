...
Bundle::Bundle(long long evt,float E_total1,float wPSD,double t0,int ts_runstart,float mE,int mSeg,float mX,float mY,float mZ,float mPSD,std::vector<int>& vSeg, std::vector<float>& vE, std::vector<float>& vZ, std::vector<float>& vPSD)
  : evt(evt)
...
Bundle::Bundle(Bundle* bundle)
  ...
  , prompt_local_E (bundle->prompt_local_E)
  , E_other (bundle->E_other)
{
}
...
void Bundle::PID(){
  ...
  if (!flag_muon){
    ...
    for (size_t i=0;i!=vE.size();i++){
      ...
      // if (evt == 377600)
      // 	std::cout << i << " " << vE.size() << " " << geometry.is_dead_seg(vSeg.at(i)) << " " << geometry.is_veto_seg(vSeg.at(i)) << " " << vE.at(i) -0.01*units::MeV << " " << vSeg.at(i)-geometry.min_SegNo << " " << vSeg.at(i)-geometry.max_SegNo << " " << vZ.at(i) - fid_z_cut.first << " " << vZ.at(i) - fid_z_cut.second << std::endl;
	...
	//      if (evt == 776250)
	//	std::cout << vE.at(i) << " " << IBD_d_E.first << " " << IBD_d_E.second << " " << vPSD.at(i) << n_PSD.first << " " << n_PSD.second << std::endl;
	// if (evt == 776248)
	// 	std::cout <<vE.at(i) << " " << vPSD.at(i) << " " << g_PSD.first << " " << g_PSD.second << std::endl;
	...
	if (geometry.is_dead_seg(vSeg.at(i)) || geometry.is_veto_seg(vSeg.at(i))
	    ...
	    || vZ.at(i) < fid_z_cut.first || vZ.at(i) > fid_z_cut.second)
	  continue;
	if (geometry.RowColDiff(vSeg.at(i),prompt_maxseg_no)>=0){
	  prompt_local_E += vE.at(i);
	}
      }
    }

    // if (evt == 3253099)
    //   std::cout << prompt_total_E-prompt_maxseg_E << " " << prompt_total_E-prompt_local_E << " " << E_other << std::endl;
    
    if (prompt_total_E-prompt_maxseg_E>=2.0*units::MeV || prompt_total_E-prompt_local_E>=0.75*units::MeV || E_other >=1.0*units::MeV)
      flag_prompt_cand = false;
